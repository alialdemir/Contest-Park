    ...
    public class UserNotFoundIntegrationEventHandler : IIntegrationEventHandler<UserNotFoundIntegrationEvent>
    {
        ...
        public Task Handle(UserNotFoundIntegrationEvent @event)
        {
            GetRetryPolicy()
                .ExecuteAsync(async () =>
                {
                    using (var httpClient = new HttpClient() { BaseAddress = new Uri(_chatSettings.identityUrl) })
                    {
                        httpClient.DefaultRequestHeaders.Add("ClientKey", _chatSettings.ClientKey);
                        httpClient.DefaultRequestHeaders.Add("ServiceName", _chatSettings.Audience);
